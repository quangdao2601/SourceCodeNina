...
function editMan()
{
    global $d, $langadmin, $strUrl, $func, $curPage, $item, $gallery, $type, $com, $act;
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
        } else {
            /* Get gallery */
            if ($act != 'copy') {
    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('news', $data)) {
            ...
            if ($func->hasFile("file1")) {
                ...
                if ($icon = $func->uploadImage("file1", $config['news'][$type]['img_type'], UPLOAD_NEWS, $file_name)) {
                    $row = $d->rawQueryOne("select id, icon from #_news where id = ? and type = ? limit 0,1", array($id, $type));

                    if (!empty($row)) {
                        $func->deleteFile(UPLOAD_NEWS . $row['icon']);
                    }
            ...
            if (isset($config['news'][$type]['schema']) && $config['news'][$type]['schema'] == true) {
                ...
                if ($buildSchema) {
                    foreach ($config['website']['seo'] as $k => $v) {
                        ...
                        if (!empty($photoUpdate['photo'])) {
                            $url_img_news = $configBase . UPLOAD_NEWS_L . $photoUpdate['photo'];
                        } else {
            ...
            if ($savehere) {
                $func->transfer(capnhatdulieuthanhcong, "index.php?com=news&act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
            } else {
                $func->transfer(capnhatdulieuthanhcong, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl);
            }
        } else {
            if ($savehere) {
                $func->transfer(capnhatdulieubiloi, "index.php?com=news&act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            } else {
                $func->transfer(capnhatdulieubiloi, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
            }
        }
    } else {
        ...
        if ($d->insert('news', $data)) {
            ...
            if (isset($config['news'][$type]['schema']) && $config['news'][$type]['schema'] == true) {
                ...
                if ($buildSchema) {
                    foreach ($config['website']['seo'] as $k => $v) {
                        ...
                        $logo = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? limit 0,1", array('logo', 'photo_static'));
                        //Tiến hành build schema article
                        $dataSchema['schema' . $k] = $func->buildSchemaArticle($id_insert, $data['name' . $k], $configBase . UPLOAD_NEWS_L . $photoUpdate['photo'], $data['date_created'], $data['date_created'], $setting['name' . $k], $url_news, $configBase . UPLOAD_PHOTO_L . @$logo['photo'], $configBase);
            ...
            if ($act == 'save_copy') {
                if($savehere || $buildSchema) {
                    $func->transfer(saochepdulieuthanhcong, "index.php?com=news&act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id_insert);
                } else {
                    $func->transfer(saochepdulieuthanhcong, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl);
                }
            } else {
                if($savehere || $buildSchema) {
                    $func->transfer(luudulieuthanhcong, "index.php?com=news&act=edit&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id_insert);
                } else {
                    $func->transfer(luudulieuthanhcong, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl);
                }
            }
        } else {
            if ($act == 'save_copy') {
                $func->transfer(saochepdulieubiloi, "index.php?com=news&act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            } else {
                $func->transfer(luudulieubiloi, "index.php?com=news&act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            }
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        /* Lấy dữ liệu */
        $row = $d->rawQueryOne("select id, photo, icon from #_news where id = ? and type = ? limit 0,1", array($id, $type));
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "index.php?com=news&act=man&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
...
function editList()
{
    global $d, $langadmin, $strUrl, $func, $curPage, $item, $gallery, $type, $com;

    $id = (!empty($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_list where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        } else {
            /* Get gallery */
...
function saveList()
{
    ...
    if ($id) {
        ...
        if ($d->update('news_list', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_list', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteList()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "index.php?com=news&act=man_list&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
...
function editCat()
{
    global $d, $langadmin, $strUrl, $func, $curPage, $item, $type;

    $id = (!empty($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_cat where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveCat()
{
    ...
    if ($id) {
        ...
        if ($d->update('news_cat', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_cat', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteCat()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(xoadulieubiloi, "index.php?com=news&act=man_cat&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
...
function editItem()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_item where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveItem()
{
    ...
    if ($id) {
        ...
        if ($d->update('news_item', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
        } else $func->transfer(capnhatdulieubiloi, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_item', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
        } else $func->transfer(luudulieubiloi, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    }
}
...
function deleteItem()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
        } else $func->transfer(xoadulieubiloi, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
        ...
        $func->transfer(xoadulieuthanhcong, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl);
    } else $func->transfer(khongnhanduocdulieu, "index.php?com=news&act=man_item&type=" . $type . "&p=" . $curPage . $strUrl, false);
}
...
function editSub()
{
    ...
    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_sub where id = ? and type = ? limit 0,1", array($id, $type));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function saveSub()
{
    ...
    if ($id) {
        ...
        if ($d->update('news_sub', $data)) {
            ...
            $func->transfer(capnhatdulieuthanhcong, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(capnhatdulieubiloi, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();
        ...
        if ($d->insert('news_sub', $data)) {
            ...
            $func->transfer(luudulieuthanhcong, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer(luudulieubiloi, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteSub()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl);
        } else $func->transfer(xoadulieubiloi, "index.php?com=news&act=man_sub&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
